...
import createOffscreenCanvas from "../utils/offscreenCanvasShim.js";
import {
  setNumberOfTrackedImages,
  _deactivateDetectionImage,
  deactivateDetectionImage,
  destroyDetectionImage,
  _destroyDetectionImage,
  activateDetectionImage,
  _activateDetectionImage,
  _createDetectionImages,
  createDetectionImage,
  _createDetectionImage,
...
export default class ARKitWrapper extends EventTarget {
  constructor() {
    ...
    // ImageTracking.js
    this.setNumberOfTrackedImages = setNumberOfTrackedImages.bind(this);
    this._deactivateDetectionImage = _deactivateDetectionImage.bind(this);
    this.deactivateDetectionImage = deactivateDetectionImage.bind(this);
    this._activateDetectionImage = _activateDetectionImage.bind(this);
    this.activateDetectionImage = activateDetectionImage.bind(this);
    this._destroyDetectionImage = _destroyDetectionImage.bind(this);
    this.destroyDetectionImage = destroyDetectionImage.bind(this);
    this._createDetectionImages = _createDetectionImages.bind(this);
    this._createDetectionImage = _createDetectionImage.bind(this);
    this.createDetectionImage = createDetectionImage.bind(this);
    this.getImageTrackingResults = getImageTrackingResults.bind(this);
    this.getTrackedImageScores = getTrackedImageScores;

    if (ARKitWrapper.HasARKit() === false) {
      console.error("ARKit not found. Variant Launch will not work."); // don't throw error so test envs will work
    }
    ...
    window["setNativeTime"] = (detail) => {
      ...
      // window.VLaunch.log("Native time: " + detail.nativeTime + ", new timeOffset: " + this._timeOffset)
    };
  ...
  _sendMessage(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (window.webkit && window.webkit.messageHandlers) {
        ...
        // NOTE: Warning here instead of throwing an error allows our testing suite to run in non-ARKit environments
        console.warn(
    ...
    if (data.removedObjects.length) {
      for (let i = 0; i < data.removedObjects.length; i++) {
        ...
        if (anchor) {
          ...
          //HAX - happens often currently, we should report this internally for logging
  ...
  createAnchorFromHit(result) {
    return new Promise((resolve, reject) => {
      const hit = result._hit;
      //console.log("hit", hit);
      if (hit.anchor_transform) {
        ...
        if (!anchor) {
          ...
          window.VLaunch.log("created dummy anchor (not plane) from hittest");
    ...
    return new Promise((resolve, reject) => {
      ...
      this._requestSession(newOptions, callbackName).then((results) => {
        if (!results.webXRAccess) {
          reject(
            new Error("user did not give permission to start a webxr session")
          );
          return;
        }
        this._waitingForSessionStart = false;
