...
import GamepadXRInputSource from "../extensions/GamepadXRInputSource.js";

import {
  getTrackedImageScores,
  getImageTrackingResults,
} from "./ImageTracking.js";
...
export default class ARKitDevice extends XRDevice {
  constructor(global) {
    ...
    this._hackySessionLock = false;

    // Image Tracking
    this._trackedImages = [];
    this.getTrackedImageScores = getTrackedImageScores.bind(this);
    this.getImageTrackingResults = getImageTrackingResults.bind(this);
    ...
    this._floorOffsetMatrix = mat4.create();
    ...
    this._stageMatrix[13] = 1.3; // HAX - magic value to match ARKit's stage height
    ...
    this._gamepads.push(createGamepad("", "none", 1, false));
    ...
    document.addEventListener("touchstart", (event) => {
      ...
      // normalize the touch position to [-1, 1] and assign to gamepad axes
      const gamepad = this._gamepads[0];
      gamepad.axes[0] = (x / window.innerWidth) * 2 - 1;
      gamepad.axes[1] = (y / window.innerHeight) * 2 - 1;
  ...
  addTransientHitTestSource(source) {
    if (!this._transientHitTestSources.includes(source)) {
      this._transientHitTestSources.push(source);
    }
  }
  ...
  _runHitTest() {
    ...
    for (const source of this._hitTestSources) {
      ...
      // 0..1 (0,0 is at top left and 1,1 is at bottom right)
      this._arKitWrapper
        .hitTest(x, y, ARKitWrapper.HIT_TEST_TYPE_EXISTING_PLANE_USING_GEOMETRY)
        .then((hits) => {
          if (!this._hitTestResultsForNextFrame.has(source)) {
  ...
  // TODO - merge back with normal hit test once I'm more familiar with the differences
  ...
  _runTransientHitTest() {
    ...
    let sourceNum = 0;
    for (let i = 0; i < this._transientHitTestSources.length; i++) {
      const source = this._transientHitTestSources[i];
      if (source._active) {
        if (source._session[XRSESSION_PRIVATE].ended) {
          source.cancel();
        } else {
          this._transientHitTestSources[sourceNum++] = source;
          if (this._transientHitTestResultsForNextFrame.has(source)) {
            this._transientHitTestResults.set(
              source,
              this._transientHitTestResultsForNextFrame.get(source)
            );
          }
        }
      }
    }
    this._transientHitTestSources.length = sourceNum;
    ...
    for (const source of this._transientHitTestSources) {
      // Convert from ray direction to screen coordinates 0.0 - 1.0 for iOS HitTest API.
      const proj = vec3.create();
      proj[0] = source._offsetRay.direction.x;
      proj[1] = source._offsetRay.direction.y;
      ...
      //will always be center-screen with default XRRay
      let x = (proj[0] + 1.0) * 0.5;
      let y = (-proj[1] + 1.0) * 0.5;
      /*
      window.VLaunch.log("old transient hit test", x, y);

      x = this._touches[0].x / 2 + 0.5;
      y = this._touches[0].y / 2 + 0.5;

      window.VLaunch.log("new transient hit test", x, y);
      */
      ...
      // Then hit test will be one frame behind but we think it's acceptable so far.
      // 0..1 (0,0 is at top left and 1,1 is at bottom right)
      // TODO - we need to store inputSource gamepad axes for x/y for transient inputs

      //HAX - reuse the existing hit test API for now
      this._arKitWrapper
        .hitTest(x, y, ARKitWrapper.HIT_TEST_TYPE_EXISTING_PLANE_USING_GEOMETRY)
        .then((hits) => {
          if (!this._transientHitTestResultsForNextFrame.has(source)) {
            this._transientHitTestResultsForNextFrame.set(source, []);
          }
          const results = this._transientHitTestResultsForNextFrame.get(source);
  ...
  getTransientHitTestResults(transientHitTestSource) {
    if (!this._transientHitTestResults.has(transientHitTestSource)) return [];
    const hits = this._transientHitTestResults.get(transientHitTestSource);
    return hits;
  }
  ...
  isFeatureSupported(featureDescriptor) {
    switch (featureDescriptor) {
      ...
      // @TODO: need to support local-floor, bounded and unbounded. local-floor is semi-supported currently
      case "local-floor":
        return true;
      //unsupported
      case "bounded":
  ...
  async setDetectionImages(trackedImages) {
    if (this._arKitWrapper) {
      //trackedImages each must have widthInMeters & width/height must be > 100 pixels
      trackedImages.forEach((image) => {
        if (image.widthInMeters === undefined || image.widthInMeters === null) {
          throw new Error("trackedImages must have widthInMeters set", image);
        }
      ...
      this._trackedImages = await this._arKitWrapper._createDetectionImages(
        trackedImages
      ); //promise
  ...
  async requestSession(mode, enabledFeatures) {
    ...
    if (this._activeSession !== null) {
      console.error("Tried to start a second session while first is running.");
      return Promise.reject();
    }
    ...
    //added for launch to get close to official WebXR spec
    if (enabledFeatures.has("camera-access-launch")) {
    ...
    if (enabledFeatures.has("image-tracking")) {
      ARKitOptions.imageTracking = true;
    }
    ...
    const watchResult = await this._arKitWrapper
      ...
      .then(async (results) => {
        ...
        if (ARKitOptions.imageTracking) {
          window.VLaunch.log("pre _createDetectionImages", this._trackedImages);
          this._trackedImages = await this._arKitWrapper._createDetectionImages(
            this._trackedImages
          );
          window.VLaunch.log(
  ...
  onBaseLayerSet(sessionId, layer) {
    //HAX - this currently gets called twice. cant find why but need to prevent changes
    if (this._hackySessionLock == true) {
      return;
    }
    const session = this._sessions.get(sessionId);
    ...
    if (oldLayer !== null) {
      ...
      if (oldCanvas.style) {
        //offscreen canvas will not have style
    ...
    if (document.body) {
      //document body can be null in iframes
    ...
    this._hackySessionLock = true;

    for (var i = 0; i < children.length; i++) {
      var child = children[i];
      var isChildOfDomOverlay = false;
      if (this._domOverlayRoot && this._domOverlayRoot.contains(child)) {
        isChildOfDomOverlay = true;
      }
      //console log all values in the if statement
      if (
        child != this._wrapperDiv &&
        child != canvas &&
        child.id !== "eruda" && //don't remove the custom console logger
        !isChildOfDomOverlay
      ) {
  ...
  endSession(sessionId) {
    ...
    if (session.baseLayer !== null) {
      // TODO - can't recall what this check is for, do we need it?
      var children = document.body.children;
      for (var i = 0; i < children.length; i++) {
        var child = children[i];
        if (child != this._wrapperDiv) {
          if (child._displayChanged == true) {
            if (child._displayWas) {
  ...
  getInputPose(inputSource, coordinateSystem, poseType) {
    for (let i = 0; i < this._gamepadInputSources.length; i++) {
      ...
      if (
        ...
        const viewMatrixInverse = mat4.invert(
          mat4.create(),
        ...
        // HAX - we are forcing a 'local' type experience, where there is no floor-level. This can cause issue for other types

        mat4.copy(this._floorOffsetMatrix, matrix);
        /*
        // get session type. if local-floor offset by stage height
        if (this._frameSession.enabledFeatures.has("local-floor")) {
          this._floorOffsetMatrix.y -= this._stageMatrix[13]; // subtract the stage height if local-floor
        }
        */

        mat4.getTranslation(this._floorOffsetMatrix, matrix);
...
class ARWatcher extends ARKitWatcher {
  ...
  handleARKitInit() {
    ...
    let event = new CustomEvent("vlaunch-ar-status", {
  ...
  handleArTrackingChanged(e) {
    ...
    let event = new CustomEvent("vlaunch-ar-tracking", { detail: e.detail });
    dispatchEvent(event);
  }
