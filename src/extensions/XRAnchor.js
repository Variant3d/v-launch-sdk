...
export default class XRAnchor extends EventTarget {
  constructor(transform, uid = null, timestamp = 0) {
    ...
    // TODO - check the use of these, both private and public assignment, and use of mat4.clone
    this._anchorSpace = new XRSpace("anchor");
    this._anchorSpace._baseMatrix = mat4.clone(transform);
    this._anchorSpace.baseMatrix = mat4.clone(transform);

    const inverse = mat4.create();
    mat4.invert(inverse, transform);
    this._anchorSpace._inverseBaseMatrix = mat4.clone(inverse);
    this._anchorSpace.inverseBaseMatrix = mat4.clone(inverse);
  }
  ...
  delete() {
    //HAX: until we rewrite the anchor handling
